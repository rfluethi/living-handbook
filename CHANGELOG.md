...
* Fixed: a MkDocs project was imported flat, with file names as titles, whenever its mkdocs.yml configured a Python plugin. The recommended Mermaid setup writes !!python/name:pymdownx.superfences.fence_code_format, a built-in YAML tag no PHP parser resolves, so Symfony YAML threw and the whole file, nav included, was discarded without a word. The nav block is now read on its own when the file as a whole cannot be parsed, and the import report says which of the two happened. A ZIP whose mkdocs.yml yields no nav says so too, instead of quietly importing the wrong structure.
