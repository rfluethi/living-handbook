...
- **Navigation**: a self-contained, collapsible page tree per handbook, built from the page hierarchy and styled by the plugin (no other plugin required). The whole block is a native `<details>` whose title toggles it; a Menu display shows the whole tree, an Accordion display collapses branches. The assembled markup is cached per handbook and rebuilt when a page or handbook term changes. Optionally the accessible handbooks are injected into the theme's own Navigation block via the `has-handbook-menu` class.
