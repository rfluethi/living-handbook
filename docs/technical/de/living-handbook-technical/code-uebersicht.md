...
- **`Export/`** ist der Website-Export: `StaticSite.php` plant einen Auftrag, rendert ihn in Durchgängen mit Zeitbudget und packt das ZIP; `SiteRenderer.php` baut das HTML, den Seitenbaum, das Inhaltsverzeichnis, den Suchindex und das Stylesheet, das im Export das Theme ersetzt.
