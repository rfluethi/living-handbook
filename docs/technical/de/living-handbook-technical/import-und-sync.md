...
- **Die Seite ist die Vorlage, kein hier erfundenes Layout.** Bis 0.73.0 baute der Export seine Seite selbst zusammen und ignorierte, was der Website-Editor sagte. Jetzt rendert er `living-handbook//single-handbook`, die bearbeitete Fassung, wenn es eine gibt, und zwar innerhalb einer echten Einzelseiten-Abfrage: die halben Blöcke fragen `is_singular()` oder lesen den aktuellen Beitrag und geben ausserhalb einer Abfrage stillschweigend gar nichts zurück. Was nicht mitreisen kann, wird vorher aus dem Markup geschnitten statt gerendert und versteckt: die Template-Teile für Kopf und Fuss des Themes, die Feedback-Frage, die Kommentare und die Schnellsuche. Der Inhaltsverzeichnis-Block wird beim Export aus den Überschriften-Ankern gefüllt, damit er ohne JavaScript und auf Papier funktioniert, und die Personen fallen aus der Metadaten-Fusszeile.
- **Das Stylesheet ist das der Website.** `wp_get_global_stylesheet()` liefert Palette, Schriften, Abstände und Layout-Regeln des Themes als `--wp--preset--*`-Eigenschaften, also genau das, worauf das Stylesheet des Plugins zurückfällt; das Kern-Blockstylesheet kommt mit, sonst sind zwei Spalten zwei gestapelte divs; und die Block-Supports, die die Style-Engine beim Rendern sammelt, werden je Seite eingesammelt und einmal geschrieben. Jede URL darin wird gegen diese Installation aufgelöst, nach `assets/site/` kopiert und umgeschrieben, so kommen die Schriften des Themes bei einer Leserin ohne Netz an.
