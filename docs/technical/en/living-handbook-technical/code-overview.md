...
- **Taxonomy**: a way to classify pages. Besides the handbook grouping there are four classifying taxonomies: page type, topic, responsible role, and audience. On the front end they are called the classification, collectively. These drive the filters and the badges.
...
- **`Export/`** is the static website export: `StaticSite.php` plans a job, renders it in passes with a time budget and packs the ZIP; `SiteRenderer.php` builds the HTML, the page tree, the table of contents, the search index and the stylesheet that stands in for the theme; `SiteThemes.php` holds the looks that stylesheet can be given, each one a handful of `--lh-user-*` properties; `TemplateRender.php` renders a page through the site's own block template, inside a query the blocks believe.
...
- **`Setup/`** is the first-run and settings code. `Seeder.php` fills the four taxonomies with starter terms on activation; `Onboarding.php` creates the overview page and the welcome notice; `Settings.php` is the settings screen (sync frequency, uninstall behaviour).
...
- English is the language of the repository; the German source of these developer docs (`docs/technical/de/`) lives in the team's workspace. The German app handbook is the exception: it ships in the repository, under `docs/user/de/`.
